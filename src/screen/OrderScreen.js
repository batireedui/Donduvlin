...
const OrderScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        let mount = true;
        axios.post(serverUrl + 'get_info.php')
            .then(data => {
                if (mount) {
                    setFlash(data.data);
                }
    ...
    let filterBook = books.filter(el => el.name.toLowerCase().includes(findValue.toLowerCase())); // Хайлт
                ...
                axios.post(serverUrl + 'insertapi.php',
                    ...
                    .then(data => {
                        if (data.data.indexOf("okok") > -1) {
                            okok = 0;
                            setModalVisible(false);
                            navigation.navigate("SuccessScreen", { utga: data.data.substring(4, data.data.length), une: parseInt(une) + parseInt(price) })
                        }
                        else {
        ...
        connectStatus ? (
            <View>
                ...
                <View style={styles.rowd}>
                    <Text style={{
                        padding: 10, alignItems: 'flex-start', flex: 3
                    }}>Сонгосон ном: {selectbook}</Text>

                    <Text style={{
                        padding: 10, alignSelf: 'flex-end', flex: 2
                    }}>Дүн: {une}</Text>
                    <TouchableOpacity
