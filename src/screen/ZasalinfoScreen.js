...
const ZasalinfoScreen = (props) => {
    ...
    useEffect(() => {
        setHend("ЗАСАЛ: Жил:" + props.route.params.jilner + ", Төрсөн он: " + props.route.params.on + ", Хүйс: " + props.route.params.huis);
        getData();
        return () => {
        }
    }, [])
    ...
    useEffect(() => {
        let mount = true;
        axios.post(serverUrl + 'price.php')
            .then(data => {
                if (mount) {
    ...
    useEffect(() => {
        ...
        let mount = true;
        setloading(true);
        axios.post(serverUrl + 'zasaltailbar.php', {
            ...
            .then(data => {
                ...
                if (mount) {
                    setBooks(Object.values(data.data[2]));
                    setHens(null);
                    setune(data.data[3]);
                    settai(data.data[1][0]);
                    setmain(data.data[0][0]);
                    setselectbook(Object.values(data.data[2]).length);
                    setinfo(null);
                ...
                axios.post(serverUrl + 'insertapi.php',
                    ...
                    .then(data => {
                        ...
                        if (data.data.indexOf("okok") > -1) {
                            ...
                            setModalVisible(false);
                            setInsertW(false);
                            props.navigation.navigate("SuccessScreen", { utga: data.data.substring(4, data.data.length), une: parseInt(une) + parseInt(price) })
                        }
                        else {
                    ...
                    .catch(err => { console.log(err); okok = 0; setInsertW(false); setinfo("Алдаа гарлаа! Дахин оролдоно уу.") });
            }
            else
                console.log(okok);
        }
    ...
    return (
        connectStatus ? (
            <ScrollView>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', margin: 15, color: '#C9302C' }}>{props.route.params.title}</Text>
                <View style={styles.headTop}>
                    <FontAwesome name="user" size={24} color="#fff" />
                    <Text style={{ color: '#fff', margin: 10, fontWeight: 'bold', }}>Жил: {props.route.params.jilner}, Төрсөн он: {props.route.params.on}, Хүйс: {props.route.params.huis}</Text>
                </View>
                {tai !== null ?
                    typeof tai.jiltailbar !== "undefined" ?
                        <View style={styles.headl}>
                            <Text style={{ color: '#fff', }}>{tai.jiltailbar}</Text>
                        </View> : null : null}
                {main !== null ?
                    <View style={styles.head}>
                        <Text style={styles.tg}>Мэнгэ (Төрсөн: {main.mengetursun} Эрхшээсэн: {main.mengeerh})</Text>
                        <Text style={styles.tg}>Суудал: {main.suudal}</Text>
                        <Text style={styles.tp}>{tai.suudaltailbar}</Text>
                        <Text style={styles.tg}>Амины чулуу: {main.ami}</Text>
                        <Text style={styles.tp}>{tai.amitailbar}</Text>
                        <Text style={styles.tg}>Биеийн чулуу: {main.biy} </Text>
                        <Text style={styles.tp}>{tai.bietailbar}</Text>
                        <Text style={styles.tg}>Эрх баялгийн чулуу: {main.erhten}</Text>
                        <Text style={styles.tp}>{tai.erhtailbar}</Text>
                        <Text style={styles.tg}>Хийморийн чулуу: {main.hiimori}</Text>
                        <Text style={styles.tp}>{tai.hiimoritailbar}</Text>
                        <Text style={styles.tg}>Сүлдний чулуу: {main.suld}</Text>
                        <Text style={styles.tp}> {tai.suldtailbar}</Text>
                    </View>
                    : null
                }
                ...
                <View style={{ paddingHorizontal: 10, alignItems: 'flex-end' }}>
                    <Text style={{ fontSize: 10 }}>Онлайн захиалгын {price} төгрөг нэмэгдсэн</Text>
                    <TouchableOpacity
                ...
                >
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            ...
                            >
                                <Text style={{ color: '#000', fontWeight: "bold", padding: 10 }}>X</Text>
                            </TouchableOpacity>
                            <Text style={[styles.modalText, { fontWeight: 'bold' }]}>ТАНЫ АЙЛТГАЛУУД</Text>
                            <View style={{ maxHeight: 100 }}>
                                <ScrollView>
                                    {
                                        aildNom !== null ?
                                            aildNom.map(el => (<Text key={el.name}>{el.name}</Text>)) : null
                                    }
                                </ScrollView>
                            </View>
                            ...
                            />
                            <Text style={{ fontSize: 10, marginBottom: 10, color: 'red' }}>{info}</Text>
                            {insertW ? <Spinning /> :
                                (<TouchableOpacity
                                    style={styles.button}
                                    onPress={insertScr}>
            ...
            </ScrollView>) : (<NoConnectionScreen onCheck={checkConnected} />)
    );
};
